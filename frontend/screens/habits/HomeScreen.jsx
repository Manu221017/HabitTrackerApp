...
export default function HomeScreen({ navigation }) {
  ...
  useEffect(() => {
    ...
    if (progress === 100 && todaysHabits.length > 0 && prev < 100) {
      setShowCelebration(true);

      // Entrada del overlay: fade + scale
      ...
      // Pequeño pulse en la barra de progreso
      Animated.sequence([
      ...
      // Ocultar celebración tras 3.5s (confetti + mensaje)
  ...
  const celebrationColors = [
    colors.primary,
    colors.habitCompleted,
    colors.accent || '#F59E0B',
    colors.habitStreak || '#8B5CF6',
    '#fff',
  ].filter(Boolean);
  ...
  return (
    <SafeAreaView style={GlobalStyles.safeArea}>
      <Modal
        ...
        animationType="none"
        onRequestClose={() => {
          Animated.timing(celebrationOpacity, { toValue: 0, duration: 200, useNativeDriver: true }).start(() => setShowCelebration(false));
        }}
      >
        <Pressable
        ...
        >
          {showCelebration && (
            <ConfettiCannon
              count={180}
              origin={{ x: width / 2, y: Dimensions.get('window').height * 0.35 }}
              explosionSpeed={400}
              fallSpeed={3500}
              fadeOut
          ...
          >
            <View style={[styles.celebrationCard, { backgroundColor: colors.cardBackground, borderColor: colors.primary + '40' }]}>
              <Text style={[styles.celebrationEmoji, { color: colors.textPrimary }]}>🎉</Text>
              <Text style={[styles.celebrationTitle, { color: colors.textPrimary }]}>¡Día completo!</Text>
              <Text style={[styles.celebrationSubtitle, { color: colors.textSecondary }]}>
                Completaste todos tus hábitos de hoy.{'\n'}¡Sigue así!
              </Text>
              <View style={[styles.celebrationBadge, { backgroundColor: colors.habitCompleted + '20' }]}>
                <Text style={[styles.celebrationBadgeText, { color: colors.habitCompleted }]}>✓ {todaysHabits.length} hábitos</Text>
              </View>
            </View>
          </Animated.View>
        </Pressable>
      ...
      >
        ...
        <View style={[GlobalStyles.card, { marginBottom: 12 }]}>
          ...
          <Animated.View style={[GlobalStyles.progressContainer, { transform: [{ scaleX: progressBarPulse }, { scaleY: progressBarPulse }] }]}>
            <Animated.View 
              style={[
